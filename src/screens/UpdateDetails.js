...
export default function ({ navigation }) {
  ...
  const sendUpdate = async (newData) => {
    await axios.post("http://192.168.136.158:3002/modifyJson", newData, {
      headers: {
        ...
        if (isConsultancy) {
          folders.map(async folderName => {
            ...
            await axios.post("http://192.168.136.158:3002/modifyJson", sendData, {
              headers: {
                'Content-Type': 'application/json'
              },
            }).then(async response => {
  ...
  const updateDetails = async () => {
    ...
    await axios.post("http://192.168.136.158:3002/nameFolders", verifyData, {
      headers: {
        'Content-Type': 'application/json'
      },
    }).then(async response => {
  ...
  useEffect(() => {
    if (isConsultancy) {
      ...
      axios.post('http://192.168.136.158:3002/nameFolders', sendData, {
        headers: {
          'Content-Type': 'application/json'
        },
      }).then(response => {
